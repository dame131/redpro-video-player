...
adb shell am start -W -S -n com.mr131.redplayer/.MainActivity | tee proof/launcher.txt
launcher_status=${PIPESTATUS[0]}
test "$launcher_status" -eq 0 || exit 14
launcher_ready=0
...
test "$launcher_ready" -eq 1 || exit 15
...
grep -Eq 'mResumedActivity.*com\.mr131\.redplayer/.MainActivity|topResumedActivity=.*com\.mr131\.redplayer/.MainActivity' \
  proof/device-state/launcher-activities.txt || exit 16
