...
public final class SubtitleDownloadActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle state){super.onCreate(state);setTitle("Subtitle Downloader");String video=getIntent().getStringExtra("video_name");if(video==null)video="";video=video.replaceFirst("(?i)\\.[a-z0-9]{2,5}$","").replace('.',' ');
        ...
        root.addView(LuxuryIconView.create(this,R.drawable.icon_subtitles_thick,"Subtitle emblem"));TextView title=text("SUBTITLE DOWNLOADER",24,getColor(R.color.text_primary));title.setContentDescription("Subtitle Downloader Screen 8");root.addView(title);
        ...
        Button search=button("SEARCH ENGLISH SUBTITLES");search.setOnClickListener(v->search());root.addView(search);status=text("Enter your free OpenSubtitles API key once. The app saves it only on this phone.",13,getColor(R.color.text_secondary));root.addView(status);
        ListView list=new ListView(this);adapter=new ArrayAdapter<String>(this,android.R.layout.simple_list_item_1,labels){@NonNull @Override public android.view.View getView(int p,android.view.View c,@NonNull android.view.ViewGroup g){TextView t=(TextView)super.getView(p,c,g);t.setTextColor(getColor(R.color.text_primary));t.setMinHeight(dp(58));return t;}};list.setAdapter(adapter);list.setOnItemClickListener((p,v,i,id)->download(fileIds.get(i)));root.addView(list,new LinearLayout.LayoutParams(-1,0,1));setContentView(root);
    }
    ...
    private EditText input(String hint){EditText e=new EditText(this);e.setHint(hint);e.setTextColor(getColor(R.color.text_primary));e.setHintTextColor(getColor(R.color.text_secondary));e.setBackgroundColor(getColor(R.color.surface_raised));e.setPadding(dp(12),dp(8),dp(12),dp(8));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(52));p.setMargins(0,dp(8),0,0);e.setLayoutParams(p);return e;}
    private Button button(String s){Button b=new Button(this);b.setText(s);b.setTextColor(getColor(R.color.text_primary));b.setBackgroundColor(getColor(R.color.red_player));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(52));p.setMargins(0,dp(10),0,0);b.setLayoutParams(p);return b;}
