...
public final class CloudImportActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        root.addView(LuxuryIconView.create(this,R.drawable.icon_cloud_thick,"Cloud Import emblem"));TextView title=text("GOOGLE DRIVE & CLOUD",25,getColor(R.color.text_primary));title.setContentDescription("Cloud Import Screen 17");root.addView(title);
        root.addView(text("Choose Google Drive, OneDrive, Dropbox, or another connected storage provider. The video can stream without being copied into the app.",16,getColor(R.color.text_secondary)));
        Button open=new Button(this);open.setText("OPEN CLOUD FILES");open.setTextColor(getColor(R.color.text_primary));open.setBackgroundColor(getColor(R.color.red_player));open.setOnClickListener(v->cloudPicker.launch(new String[]{"video/*","application/vnd.apple.mpegurl","application/x-mpegURL"}));
