...
public final class AboutActivity extends AppCompatActivity {
    @Override protected void onCreate(Bundle state) {
        ...
        TextView title=text("131 RED PLAYER",28,getColor(R.color.text_primary));title.setContentDescription("About and Privacy Screen");root.addView(title);
        root.addView(text("Version "+BuildConfig.VERSION_NAME,17,getColor(R.color.text_secondary)));
        root.addView(text("Private, ad-free video and music playback built from selected VLC and XPlayer-style controls.",16,getColor(R.color.text_primary)));
        root.addView(text("PRIVACY",20,getColor(R.color.red_player)));
        root.addView(text("Your local videos stay on your device unless you choose Cast, a network address, cloud storage, sharing, or subtitle download. History, bookmarks, settings, and saved addresses remain in this app. Cloud providers control their own sign-in and access.",15,getColor(R.color.text_secondary)));
        root.addView(text("4K MODE",20,getColor(R.color.red_player)));
        root.addView(text("4K Ultra HD mode selects the highest real video track available. It does not create missing picture detail or claim AI upscaling.",15,getColor(R.color.text_secondary)));
        Button clear=button("CLEAR TEMPORARY CACHE");clear.setOnClickListener(v->confirmClear());root.addView(clear);
        root.addView(text("Clearing temporary cache does not delete your personal videos, vault files, playlists, bookmarks, or settings.",13,getColor(R.color.text_secondary)));
    ...
    private Button button(String label){Button b=new Button(this);b.setText(label);b.setTextColor(getColor(R.color.text_primary));b.setBackgroundColor(getColor(R.color.red_player));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(52));p.setMargins(0,dp(20),0,dp(8));b.setLayoutParams(p);return b;}
