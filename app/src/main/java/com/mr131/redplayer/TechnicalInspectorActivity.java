...
public final class TechnicalInspectorActivity extends AppCompatActivity {
    @Override protected void onCreate(Bundle state){super.onCreate(state);setTitle("Technical Inspector");ScrollView scroll=new ScrollView(this);RedPlayerBackground.apply(scroll);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(18),dp(16),dp(18),dp(20));
        root.addView(LuxuryIconView.create(this,R.drawable.icon_info_thick,"Technical information emblem"));TextView title=text("VIDEO TECHNICAL INSPECTOR",24,getColor(R.color.text_primary));title.setContentDescription("Technical Inspector Screen 12");root.addView(title);String details=getIntent().getStringExtra("details");if(details==null)details="Open a video to inspect its resolution, frame rate, bitrate, codecs, audio channels, sample rate, decoder, duration, and location.";root.addView(text(details,16,getColor(R.color.text_secondary)));scroll.addView(root);setContentView(scroll);}
