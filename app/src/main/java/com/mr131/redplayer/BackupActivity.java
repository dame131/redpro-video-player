...
public final class BackupActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle state){super.onCreate(state);setTitle("Backup & Restore");LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setGravity(Gravity.CENTER);root.setPadding(dp(24),dp(24),dp(24),dp(24));RedPlayerBackground.apply(root);root.addView(LuxuryIconView.create(this,R.drawable.icon_cloud_thick,"Backup emblem"));TextView title=text("BACKUP & RESTORE",25,getColor(R.color.text_primary));title.setContentDescription("Backup Restore Screen");root.addView(title);root.addView(text("Save settings, history, favorites, playlists and network addresses to your phone or Google Drive.",15,getColor(R.color.text_secondary)));Button backup=button("CREATE BACKUP");backup.setOnClickListener(v->exportFile.launch("131-red-player-backup.json"));root.addView(backup);Button restore=button("RESTORE BACKUP");restore.setOnClickListener(v->importFile.launch(new String[]{"application/json","text/*"}));root.addView(restore);status=text("Ready",14,getColor(R.color.text_secondary));root.addView(status);setContentView(root);}
    ...
    private Button button(String value){Button b=new Button(this);b.setText(value);b.setTextColor(getColor(R.color.text_primary));b.setBackgroundColor(getColor(R.color.red_player));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(54));p.setMargins(0,dp(14),0,0);b.setLayoutParams(p);return b;}
