...
public final class PlaybackLabActivity extends AppCompatActivity {
 ...
 private LinearLayout screen(){
  ...
  root.addView(text("PLAYBACK LAB",26,getColor(R.color.text_primary)));root.addView(text("Fine control for difficult video, sound and subtitles",14,getColor(R.color.text_secondary)));
  CheckBox normalize=new CheckBox(this);normalize.setText("LEVEL LOUD AND QUIET AUDIO");normalize.setTextColor(getColor(R.color.text_primary));normalize.setButtonTintList(android.content.res.ColorStateList.valueOf(getColor(R.color.red_player)));normalize.setChecked(prefs.getBoolean("vlc_normalize",false));normalize.setOnCheckedChangeListener((b,on)->prefs.edit().putBoolean("vlc_normalize",on).apply());root.addView(normalize);
  boostValue=text("",15,getColor(R.color.text_primary));root.addView(boostValue);root.addView(seek("AUDIO BOOST",100,200,prefs.getInt("vlc_volume",100),(value)->{prefs.edit().putInt("vlc_volume",value).apply();boostValue.setText("AUDIO BOOST  "+value+"%");}));boostValue.setText("AUDIO BOOST  "+prefs.getInt("vlc_volume",100)+"%");
  audioDelayValue=text("",15,getColor(R.color.text_primary));root.addView(audioDelayValue);root.addView(seek("AUDIO TIMING",0,10000,prefs.getInt("vlc_audio_delay",0)+5000,(value)->{int delay=value-5000;prefs.edit().putInt("vlc_audio_delay",delay).apply();audioDelayValue.setText("AUDIO TIMING  "+signed(delay)+" ms");}));audioDelayValue.setText("AUDIO TIMING  "+signed(prefs.getInt("vlc_audio_delay",0))+" ms");
  subtitleDelayValue=text("",15,getColor(R.color.text_primary));root.addView(subtitleDelayValue);root.addView(seek("SUBTITLE TIMING",0,10000,prefs.getInt("vlc_subtitle_delay",0)+5000,(value)->{int delay=value-5000;prefs.edit().putInt("vlc_subtitle_delay",delay).apply();subtitleDelayValue.setText("SUBTITLE TIMING  "+signed(delay)+" ms");}));subtitleDelayValue.setText("SUBTITLE TIMING  "+signed(prefs.getInt("vlc_subtitle_delay",0))+" ms");
  cacheValue=text("",15,getColor(R.color.text_primary));root.addView(cacheValue);root.addView(seek("NETWORK BUFFER",0,60000,prefs.getInt("vlc_network_cache",1500),(value)->{prefs.edit().putInt("vlc_network_cache",value).apply();cacheValue.setText("NETWORK BUFFER  "+value+" ms");}));cacheValue.setText("NETWORK BUFFER  "+prefs.getInt("vlc_network_cache",1500)+" ms");
  deinterlaceValue=text("",15,getColor(R.color.text_primary));root.addView(deinterlaceValue);Button deinterlace=button("CHANGE VIDEO DEINTERLACING");deinterlace.setOnClickListener(v->{int next=(prefs.getInt("vlc_deinterlace",0)+1)%DEINTERLACE.length;prefs.edit().putInt("vlc_deinterlace",next).apply();deinterlaceValue.setText("VIDEO DEINTERLACING  "+DEINTERLACE[next]);});root.addView(deinterlace);deinterlaceValue.setText("VIDEO DEINTERLACING  "+DEINTERLACE[prefs.getInt("vlc_deinterlace",0)]);
  encodingValue=text("",15,getColor(R.color.text_primary));root.addView(encodingValue);Button encoding=button("CHANGE SUBTITLE TEXT ENCODING");encoding.setOnClickListener(v->{int next=(prefs.getInt("subtitle_encoding",0)+1)%ENCODING.length;prefs.edit().putInt("subtitle_encoding",next).apply();encodingValue.setText("SUBTITLE TEXT  "+ENCODING[next]);});root.addView(encoding);encodingValue.setText("SUBTITLE TEXT  "+ENCODING[prefs.getInt("subtitle_encoding",0)]);
 ...
 private Button button(String label){Button b=new Button(this);b.setText(label);b.setTextColor(getColor(R.color.text_primary));b.setBackgroundColor(getColor(R.color.carbon));b.setMinHeight(dp(48));return b;}
