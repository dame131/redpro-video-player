...
public final class HistoryActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle state){super.onCreate(state);setTitle("History & Recovery");prefs=getSharedPreferences("red_player",MODE_PRIVATE);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(14),dp(12),dp(14),dp(12));RedPlayerBackground.apply(root);
        root.addView(LuxuryIconView.create(this,R.drawable.icon_history_thick,"History emblem"));TextView title=text("HISTORY & RECOVERY",24,getColor(R.color.text_primary));title.setContentDescription("History Screen 18");root.addView(title);String error=prefs.getString("last_error","");root.addView(text(error.isEmpty()?"No recent playback errors":"Last recovery: "+error,13,error.isEmpty()?getColor(R.color.text_secondary):getColor(R.color.red_player)));
        Button clear=new Button(this);clear.setText("CLEAR HISTORY");clear.setTextColor(getColor(R.color.text_primary));clear.setBackgroundColor(getColor(R.color.surface_raised));clear.setOnClickListener(v->{prefs.edit().remove("history").remove("last_error").apply();entries.clear();labels.clear();adapter.notifyDataSetChanged();});root.addView(clear,new LinearLayout.LayoutParams(-1,dp(48)));
        ListView list=new ListView(this);adapter=new ArrayAdapter<String>(this,android.R.layout.simple_list_item_1,labels){@NonNull @Override public android.view.View getView(int p,android.view.View c,@NonNull android.view.ViewGroup g){TextView t=(TextView)super.getView(p,c,g);t.setTextColor(getColor(R.color.text_primary));t.setMinHeight(dp(68));return t;}};list.setAdapter(adapter);list.setOnItemClickListener((p,v,i,id)->{if(i>=entries.size())return;Uri uri=Uri.parse(entries.get(i).optString("uri"));setResult(RESULT_OK,new Intent().setData(uri).addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION));finish();});root.addView(list,new LinearLayout.LayoutParams(-1,0,1));setContentView(root);load();}
