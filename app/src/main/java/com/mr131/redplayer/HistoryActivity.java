...
public final class HistoryActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle state){super.onCreate(state);setTitle("History & Recovery");prefs=getSharedPreferences("red_player",MODE_PRIVATE);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(14),dp(12),dp(14),dp(12));RedPlayerBackground.apply(root);
        root.addView(LuxuryIconView.create(this,R.drawable.icon_history_thick,"History emblem"));TextView title=text("HISTORY & RECOVERY",24,Color.WHITE);title.setContentDescription("History Screen 18");root.addView(title);String error=prefs.getString("last_error","");root.addView(text(error.isEmpty()?"No recent playback errors":"Last recovery: "+error,13,error.isEmpty()?Color.LTGRAY:Color.rgb(214,10,0)));
