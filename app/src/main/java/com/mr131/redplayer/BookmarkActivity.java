...
public final class BookmarkActivity extends AppCompatActivity {
 ...
 private void manage(int i){new AlertDialog.Builder(this).setTitle("Bookmark options").setItems(new String[]{"Rename bookmark","Remove bookmark"},(d,w)->{if(w==0)rename(i);else remove(i);}).show();}
 private void rename(int i){EditText input=new EditText(this);input.setText(items.get(i).optString("label","Bookmark"));input.setTextColor(getColor(R.color.text_primary));new AlertDialog.Builder(this).setTitle("Rename bookmark").setView(input).setNegativeButton("Cancel",null).setPositiveButton("Save",(d,w)->{String name=input.getText().toString().trim();if(name.isEmpty()){Toast.makeText(this,"Enter a bookmark name",Toast.LENGTH_SHORT).show();return;}try{items.get(i).put("label",name);save();load();}catch(Exception ignored){Toast.makeText(this,"Bookmark could not be renamed",Toast.LENGTH_SHORT).show();}}).show();}
